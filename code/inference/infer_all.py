...
#     parser.add_argument(
#         "--gpu", default=1, help="which gpu do you want to use", type=int
#     )
#     parser.add_argument(
#         "--start_checkpoint", default=58000, help="start checkpoint", type=int
#     )
#     parser.add_argument("--end_checkpoint", default=62000, help="last checkpoint", type=int)
#     parser.add_argument(
#         "--checkpoint_step", default=2000, help="each step between checkpoint", type=int
#     )
#     parser.add_argument(
#         "--testset_token", action="append", default=["[PKU]"], help="Select the dataset"
#     )
#     parser.add_argument(
#         "--exp_name", default="PKU", help="name your experiment", type=str
#     )
#     parser.add_argument(
#         "--criteria",
#         default=10,
...
#     )
#
#     writer = SummaryWriter(f"../script/exp/result/{EXP_FILE}")
#
#     for ckpt in range(
...
#         model.eval()
#         try:
#             os.makedirs("../script/exp/result/" + EXP_FILE)
#         except:
#             pass
...
#             with torch.no_grad():
#                 f = open(
#                     f"../script/exp/result/{EXP_FILE}/{ckpt}_{dataset_name}_{args.criteria_token}.txt",
#                     "w",
#                 )
...
def main(args):
    ...
    for ckpt in range(
            ...
            with torch.no_grad():
                ...
                for data, text, short_input in test_data:
                    ...
                    for i in range(len(segmentation)):
                        ...
                        if len(ans) > 0:
                            tmp.append(ans)
                        if tmp and short_input[i] and tmp[-1][-1] != "\n":
                            tmp.append("\n")
                        f.write(" ".join(tmp))
                f.close()
        ...
        # )
        #
        # with open(f"../script/exp/result/{EXP_FILE}/F1_score.txt", "a") as f:
        #     f.write(f"checkpoint : {ckpt} F1 : {F1_score} OOV_recall : {OOV_recall}\n")
        # if args.criteria_token != "[UNC]":
        #     writer.add_scalar(f"{args.testset_token[0]}/F1_score", F1_score, ckpt)
        #     writer.add_scalar(f"{args.testset_token[0]}/OOV_recall", OOV_recall, ckpt)
